...
File.open('bus-services.json', 'r') do |bus_services_json|
  ...
  bus_services.each do |bus_service|
    bus_service['buses'].each do |bus|
      ...
      bus_routes = Nokogiri::XML(open("bus-routes/#{number}-#{direction}.kml"),&:noblanks)
      #bus_routes.remove_namespaces!
      ...
      if folder.nil? then
        puts "ERROR > Fails to find an insertion point for placemarks."
        next
      end
      bus_stops['direction'].each do |d|
        d.each do |bus_stop|
      ...
      File.open("kml/#{number}.kml", 'w') do |f|
